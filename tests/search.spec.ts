...
import { openSidebar, openWith, settled, stats } from './helpers';
...
test.describe('tracing a wording', () => {
  const trace = async (page: import('@playwright/test').Page, wording: string) => {
  ...
  test('an Arabic phrase finds the reports that carry it', async ({ page }) => {
  ...
  test('English finds the same reports through the translation', async ({ page }) => {
    await openWith(page, { books: [] });
    await openSidebar(page);
  ...
  test('the phrase-only scope narrows to the wording itself', async ({ page }) => {
    await openWith(page, { books: [] });
    await openSidebar(page);
    await trace(page, 'إنما الأعمال بالنيات');
  ...
  test('clearing a search puts the collections back', async ({ page }) => {
    await openWith(page);
    await openSidebar(page);
    ...
    await page.locator('.panel', { hasText: 'Trace a wording' }).getByRole('button', { name: 'clear' }).click();
    await settled(page);
    expect((await stats(page)).hadiths).toBe(before.hadiths);
  });

  test('a wording nothing carries says so instead of emptying the screen', async ({ page }) => {
    await openWith(page, { books: [] });
    await openSidebar(page);
  ...
  test('a long result list can be opened up', async ({ page }) => {
    await openWith(page, { books: [] });
    await openSidebar(page);
  ...
  test('a single result can be isolated from the list', async ({ page }) => {
    await openWith(page, { books: [] });
    await openSidebar(page);
    await trace(page, 'إنما الأعمال بالنيات');
  ...
  test('a search and a collection filter compose', async ({ page }) => {
    await openWith(page, { books: [] });
    await openSidebar(page);
    await trace(page, 'إنما الأعمال بالنيات');
    const everywhere = (await stats(page)).hadiths;

    // Every collection starts off here, so turning one on is a single click.
